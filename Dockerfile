...
RUN apt-get update && \
        ...
        apt-get install -y gnupg2 dirmngr vim rsyslog dialog mc lynx apt-utils && \
...
RUN apt-get install -q -y language-pack-en language-pack-fr

RUN DEBIAN_FRONTEND="noninteractive" apt-get install -y php7.1 libapache2-mod-php7.1 php7.1-mysql php7.1-cli php7.1-gd php7.1-imap php7.1-curl \
...
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends munin-node munin-plugins-extra
